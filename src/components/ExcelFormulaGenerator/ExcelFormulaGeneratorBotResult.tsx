...
import { getExcelFormula } from "./ExcelFormulaGeneratorData.ts";
import { formatResultText } from "../../utils/textutils.ts";

const ExcelFormulaGeneratorBotResult = () => {
  const { excelFormulaId } = useParams();
  ...
  useEffect(() => {
    async function fetch() {
      if (excelFormulaId) {
        const response = await getExcelFormula(excelFormulaId);
...
export default ExcelFormulaGeneratorBotResult;
