...
import { getYoutubeDescription } from "./YoutubeDescriptionGeneratorData.ts";
import { formatResultText } from "../../utils/textutils.ts";

const YoutubeDescriptionGeneratorResult = () => {
  const { youtubeDescriptionId } = useParams();
  ...
  useEffect(() => {
    async function fetch() {
      if (youtubeDescriptionId) {
        const response = await getYoutubeDescription(youtubeDescriptionId);
...
export default YoutubeDescriptionGeneratorResult;
